...
public class LabelScreenshotsFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (((AppCompatActivity)requireActivity()).getSupportActionBar() != null) {
            String title = dataStoreHelper.getStringValue(LABEL_NAME, null);
            if (title.equals("null"))
                ((AppCompatActivity) requireActivity()).getSupportActionBar().setTitle(getString(R.string.screenshot_label));
            else
                ((AppCompatActivity) requireActivity()).getSupportActionBar().setTitle(title);
        }

        viewModel.getImagesByLabel(dataStoreHelper.getIntValue(LABEL_ID, 0)).observe(getViewLifecycleOwner(), imageEntities -> {
